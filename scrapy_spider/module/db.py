...
def replace(table, **kw):
    """
    执行insert语句
    """
    cols, args = zip(*kw.iteritems())
    sql = 'replace into `%s` (%s) values (%s)' % (table, ','.join(['`%s`' % col for col in cols]), ','.join(['?' for i in range(len(cols))]))
    return _update(sql, *args)
#
#
class Dict(dict):
...
if __name__ == '__main__':
    ...
    create_engine('root', '1234', 'industry', 'localhost')
    update('create table %s (source VARCHAR(255),type VARCHAR(255),company_name VARCHAR(255),'
           'website VARCHAR(255) primary key,address VARCHAR(255),email VARCHAR(255),phone VARCHAR(255),'
           'fax VARCHAR(255),is_send VARCHAR(255),finish_person VARCHAR(255),finish_date datetime)' %'table_name')
